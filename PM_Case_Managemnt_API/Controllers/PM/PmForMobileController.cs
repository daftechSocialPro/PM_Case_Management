...
namespace PM_Case_Managemnt_API.Controllers.PM
{
    ...
    public class PmForMobileController : ControllerBase
    {
        ...
        public async Task<ActionResult> Add_ProgressWC(ProgressStatus pros, string quarterId, string activityId, float ActualBudget, float ActualWorked, string EmployeeValueId, string Remark, float lat = 0, float lng = 0)
        {
        ...
        public ActionResult getNotification(Guid empId)
        {
            ...
            var notfiyplans = _db.Plans.Where(x => (x.FinanceId == currentUser.Id || x.ProjectManagerId == currentUser.Id || (x.StructureId == currentUser.OrganizationalStructureId && currentUser.Position == Position.Director))
   ...
   ||
   (x.Tasks.Any(y => y.Activities.Any(z => z.ActProgress.Any(a => a.IsApprovedByManager == approvalStatus.pending || a == approvalStatus.pending || a.IsApprovedByFinance == approvalStatus.pending))))).ToList();
        ...
        public ActionResult GetTasks(Guid employeeId)
        {
            ...
            var tasks = _db.Tasks.Where(x => x.TaskMember.Any(y => y.EmployeeId == employeeId)).ToList();
