...
import com.example.casemanagment.Remote.AppointmentAdapter
...
class AppointmentFragment : Fragment() {
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?,
        ...
        this.requireActivity().setTitle("Appointments")
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        compositeDisposable.addAll(
            iMyAPI.GetAppointments(user!!)
                .subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread())
                .subscribe(
                    { s -> if (s.any()){
                        val recyclerView = view?. findViewById<RecyclerView>(R.id.recycler_view_appointment)
                        val adapter = AppointmentAdapter()

                        recyclerView.layoutManager= LinearLayoutManager(this.requireContext())
                        recyclerView.adapter = adapter
                        adapter.Appointments= s
                        ...
                        Toast.makeText(this.requireContext(),"No Appointments ", Toast.LENGTH_SHORT).show()
                        dialog.dismiss()
                    }},
                    { t: Throwable? ->
